...
const STEPS = {
  ...
  applicant_type: {
    ...
    why: "This changes the documents and lenders we recommend — and the income questions I ask next.",
    type: "options",
    options: ["Individual — salaried or self-employed", "Business Owner / MSME"],
  },
  cibil: {
    q: "Do you know your approximate CIBIL score?",
    why: "CIBIL is checked by every lender. It directly affects which lenders can approve and at what rate.",
    type: "options",
    options: ["750 and above", "700 – 750", "650 – 700", "Below 650", "Don't know my score"],
  },

  // INCOME QUESTIONS — smart routing
  individual_income: {
    q: "What is your net monthly take-home income?",
    why: "Your eligible EMI is typically 50% of take-home. This tells me the maximum loan you can service.",
    type: "options",
    options: ["Below ₹30,000", "₹30K – ₹60K", "₹60K – ₹1.5 Lakh", "₹1.5L – ₹3 Lakh", "Above ₹3 Lakh"],
  },
  msme_turnover: {
    q: "What is your business monthly turnover (approximate)?",
    why: "Monthly turnover is the primary income signal for most MSME lenders — more important than profits.",
    type: "options",
    options: ["Below ₹2 Lakh", "₹2L – ₹10L", "₹10L – ₹50L", "₹50L – ₹2 Crore", "Above ₹2 Crore"],
  },
  msme_income: {
    q: "And your net monthly income (salary or drawings from business)?",
    why: "Lenders look at both turnover and personal income — especially for secured loans. Both matter.",
    type: "options",
  ...
  business_vintage: {
    ...
    why: "Business age is the most checked factor by lenders — it determines which lender categories are available.",
    type: "options",
    options: ["Less than 1 year", "1–2 years", "2–3 years", "3–5 years", "More than 5 years"],
  },
  business_gst: {
    q: "Is your business GST registered?",
    why: "GST returns are primary income proof for MSME lenders. Registered businesses get better rates and higher limits.",
    type: "options",
    options: ["Yes — GST registered", "No — below threshold", "Applied but not yet registered"],
  },
  business_amount: {
    q: "How much are you looking to borrow?",
    why: "This tells me whether banks or NBFCs are the better fit for your case.",
    type: "options",
  ...
  lap_purpose: {
    ...
    why: "Purpose determines product structure — business use is assessed differently from personal requirement.",
    type: "options",
  ...
  lap_property: {
    ...
    why: "Property type is most critical — it determines which lenders will consider your case at all.",
    type: "options",
  ...
  lap_existing: {
    q: "Is there an existing loan on this property?",
    why: "Existing loan reduces available equity. Outstanding amount changes your real top-up room.",
    type: "options",
    options: ["No existing loan — clear property", "Yes — want to transfer + top up", "Yes — only top up needed"],
  },
  lap_amount: {
  ...
  hl_employment: {
    ...
    why: "Income documentation and lender options depend entirely on this.",
    type: "options",
  ...
  pl_existing_emi: {
    ...
    why: "Personal loan eligibility drops sharply if existing EMIs are high. FOIR cannot exceed 50–65%.",
    type: "options",
  ...
  ub_vintage: {
    q: "How long has your business been running?",
    why: "Fintech NBFCs accept 1+ year. Banks need 2-3 years. This determines who can approve.",
    type: "options",
    options: ["Less than 1 year", "1–2 years", "2–3 years", "More than 3 years"],
  },
  ub_gst: {
    q: "Is your business GST registered?",
    why: "Most fintech lenders calculate your eligible amount as 20-30% of annual GST turnover.",
    type: "options",
    options: ["Yes — GST registered", "No — below threshold", "Applied but not yet registered"],
  },
  ub_amount: {
  ...
  rejected_product: {
    ...
    why: "Different rejection reasons apply to different products. Product tells me where to look first.",
    type: "options",
    options: ["Home Loan", "Business Loan / Working Capital", "Personal Loan", "LAP", "Other"],
  },
  rejected_reason: {
    q: "What reason did the bank give for rejection?",
    why: "Stated reason is often not the real reason. But it gives me the starting point for diagnosis.",
    type: "options",
    options: ["Low CIBIL score", "Insufficient income / FOIR too high", "Business too new / low vintage", "Property issue", "Documents incomplete", "No reason given"],
  },
  rejected_count: {
    q: "How many lenders have rejected you?",
    why: "Multiple rejections create hard enquiries which further damage CIBIL. Critical to know before recommending next steps.",
    type: "options",
...
function buildSequence(loanType, answers) {
  ...
  const isSelfEmployed = answers.hl_employment?.includes("Self-Employed") || answers.hl_employment?.includes("Professional");

  const sequences = {
    business:  ["business_type", "business_vintage", "msme_turnover", "msme_income", "business_gst", "business_amount", "cibil"],
    property:  [
      "lap_purpose", "applicant_type", "lap_property", "lap_value", "lap_existing", "lap_amount",
      ...(isMSME ? ["msme_turnover", "msme_income"] : ["individual_income"]),
      "cibil"
    ],
    home:      [
      "hl_employment",
      ...(isSelfEmployed ? ["msme_turnover", "msme_income"] : ["individual_income"]),
      "hl_existing_emi", "hl_amount", "cibil"
    ],
    personal:  ["applicant_type",
      ...(isMSME ? ["msme_turnover", "msme_income"] : ["individual_income"]),
      "pl_existing_emi", "pl_amount", "cibil"
    ],
    unsecured: ["ub_vintage", "msme_turnover", "msme_income", "ub_gst", "ub_amount", "cibil"],
    rejected:  ["rejected_product", "rejected_reason", "rejected_count", "cibil"],
  };
  return sequences[loanType] || [];
}

// ── INSIGHTS ───────────────────────────────────────────────────────────────
const INSIGHTS = {
  business_vintage: {
    "Less than 1 year": { msg: "Very new business — noted. Fintech lenders like Lendingkart and FlexiLoans work with early-stage businesses. We will find the right path.", color: "#D97706" },
    "1–2 years":        { msg: "1-2 year vintage opens several NBFC options. With the right structuring this is workable.", color: "#D97706" },
    "3–5 years":        { msg: "Good vintage — most private banks and NBFCs will consider your case.", color: GREEN },
    "More than 5 years":{ msg: "Excellent — 5+ years gives you access to the full lender panel including PSU banks.", color: GREEN },
  },
  lap_property: {
    ...
    "Commercial Property":          { msg: "Commercial property typically gets 50–60% LTV. Some lenders specialise here — we will find the right one.", color: "#D97706" },
    "Residential — Society / Apartment": { msg: "Clean residential property — best LTV possible. Strong case for most lenders.", color: GREEN },
    "Residential — Independent House":   { msg: "Good. Independent residential property is accepted by most lenders.", color: GREEN },
  },
  cibil: {
    "750 and above":       { msg: "Excellent CIBIL — you qualify for the best rates from all lenders.", color: GREEN },
    "700 – 750":           { msg: "Good score. Most lenders will consider you. Rate may be slightly higher than top tier.", color: GREEN },
    "650 – 700":           { msg: "Borderline — lender selection becomes critical. We know which ones to approach.", color: "#D97706" },
    "Below 650":           { msg: "Low CIBIL — but not the end. NBFCs have higher risk appetite. We can also build an improvement plan.", color: "#D97706" },
    "Don't know my score": { msg: "No problem — our assessment will include a CIBIL check recommendation.", color: INDIGO },
  },
  rejected_count: {
    "3 or more lenders": { msg: "STOP applying now. Each rejection creates a hard enquiry that further damages your CIBIL. Let us diagnose first.", color: "#DC2626" },
    "2 lenders":         { msg: "Two rejections mean a specific issue needs identifying before approaching a third lender.", color: "#D97706" },
    "Only 1 lender":     { msg: "One rejection is common. Different lenders have completely different appetite.", color: GREEN },
  },
  applicant_type: {
    "Business Owner / MSME": { msg: "MSME assessment — I will look at both your business turnover and personal income separately.", color: INDIGO },
    "Individual — salaried or self-employed": { msg: "Individual assessment — your monthly take-home income is the key number.", color: INDIGO },
  },
};
...
export default function Check({ navigate }) {
  ...
  // Rebuild sequence whenever answers change (dynamic routing)
  ...
  // Detect when we have passed all questions
  useEffect(() => {
    if (phase === "flow" && loanType && stepIdx >= sequence.length && sequence.length > 0) {
      setPhase("contact");
    }
  ...
  const handleAnswer = (val) => {
    ...
    const flash = stepInsights?.[val];
    if (flash) setInsight(flash);
      ...
      <div style={{ maxWidth: 740, margin: "0 auto", padding: "clamp(32px,5vw,52px) 20px" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <h1 style={{ fontSize: "clamp(22px,4vw,30px)", fontWeight: 900, color: DARK, marginBottom: 10, lineHeight: 1.25 }}>
            ...
            <span style={{ color: INDIGO }}>We find you the best path.</span>
          </h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 12 }}>
          {LOAN_CARDS.map(c => (
            <div key={c.id} onClick={() => handleLoanType(c.id)}
              style={{ background: WHITE, border: "2px solid #E5E7EB", borderRadius: 12, padding: "20px 16px", cursor: "pointer" }}
  ...
  if (phase === "flow" && currentStep) return (
    <div style={{ fontFamily: "Arial,sans-serif", minHeight: "100vh", background: GRAY }}>
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "clamp(24px,4vw,44px) 20px" }}>
        ...
        {/* Insight flash */}
        {insight && (
          <div style={{ background: insight.color + "15", border: `1px solid ${insight.color}40`, borderRadius: 10, padding: "10px 14px", marginBottom: 16, display: "flex", gap: 10 }}>
            <span>💡</span>
        ...
        <div style={{ background: WHITE, borderRadius: 14, padding: "24px 20px", border: "1px solid #E5E7EB", boxShadow: "0 4px 16px rgba(79,70,229,0.06)" }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: DARK, marginBottom: 8, lineHeight: 1.3 }}>
            {currentStep.q}
          </div>
          {currentStep.why && (
            <div style={{ fontSize: 12, color: MUTED, marginBottom: 18, lineHeight: 1.5, fontStyle: "italic", borderLeft: `3px solid ${ORANGE}`, paddingLeft: 10 }}>
              {currentStep.why}
            </div>
          )}
  ...
  if (phase === "contact") return (
    <div style={{ fontFamily: "Arial,sans-serif", minHeight: "100vh", background: GRAY }}>
      <div style={{ maxWidth: 540, margin: "0 auto", padding: "clamp(24px,4vw,44px) 20px" }}>
        ...
        {/* Summary */}
        <div style={{ background: WHITE, borderRadius: 14, padding: "20px", border: "1px solid #E5E7EB", marginBottom: 16, boxShadow: "0 4px 16px rgba(79,70,229,0.06)" }}>
          ...
          {Object.entries(answers).map(([k, v]) => (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "5px 0", borderBottom: "1px solid #F3F4F6", gap: 12 }}>
              <span style={{ fontSize: 11, color: MUTED, flexShrink: 0 }}>{STEPS[k]?.q?.replace("?","") || k}</span>
              <span style={{ fontSize: 11, fontWeight: 600, color: DARK, textAlign: "right" }}>{v}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: DARK, borderRadius: 14, padding: "24px 20px" }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input type="text" placeholder="Ramesh Sharma"
              value={contact.name} onChange={e => { setContact(c => ({ ...c, name: e.target.value })); setErrors(e2 => ({ ...e2, name: null })); }}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input type="tel" placeholder="98xxxxxxxx" maxLength={10}
              value={contact.mobile} onChange={e => { setContact(c => ({ ...c, mobile: e.target.value })); setErrors(e2 => ({ ...e2, mobile: null })); }}
  ...
  if (phase === "result") return (
    <div style={{ fontFamily: "Arial,sans-serif", minHeight: "100vh", background: GRAY }}>
      <div style={{ maxWidth: 540, margin: "0 auto", padding: "clamp(32px,5vw,52px) 20px", textAlign: "center" }}>
        ...
        <button onClick={() => window.open(`https://wa.me/919579453635?text=Hi%20FinsightOne%2C%20I%20just%20submitted%20my%20eligibility%20check%20for%20${encodeURIComponent(loanType || "loan")}%20-%20${encodeURIComponent(contact.name)}`, "_blank")}
