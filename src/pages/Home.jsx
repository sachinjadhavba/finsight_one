  ...
  return (
    <div>
      ...
      <div style={{ background: "linear-gradient(135deg,#1E3A5F 0%,#0F2140 100%)", color: "#fff", padding: "clamp(40px,6vw,72px) clamp(20px,4vw,48px) clamp(40px,5vw,64px)", textAlign: "center" }}>
        <div style={{ display: "inline-block", background: "rgba(180,83,9,0.2)", border: "1px solid rgba(180,83,9,0.4)", color: "#FCD34D", fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", padding: "5px 16px", borderRadius: 100, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 40 }}>
          ...
          <button onClick={() => navigate("about")} style={{ background: "transparent", color: "#fff", fontSize: "clamp(13px,1.5vw,14px)", fontWeight: 600, padding: "14px 28px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.25)", cursor: "pointer" }}>
            See All Services
          </button>
        </div>
        <div style={{ display: "flex", justifyContent: "center", gap: "clamp(20px,4vw,48px)", flexWrap: "wrap" }}>
          {HERO_STATS.map(s => (
            <div key={s.label} style={{ textAlign: "center" }}>
              <div style={{ fontSize: "clamp(18px,2vw,24px)", fontWeight: 800, color: "#FCD34D" }}>{s.val}</div>
      ...
      {/* ── PROBLEM STRIP ── */}
      <div style={{ background: "#FEF3C7", padding: "14px clamp(16px,3vw,48px)", display: "flex", gap: "clamp(10px,2vw,24px)", justifyContent: "center", flexWrap: "wrap", borderBottom: "1px solid #FDE68A" }}>
        {["❌ Loan rejected?", "📄 Wrong documents?", "📉 Don't know why refused?", "⏳ Waiting months?"].map(t => (
          <div key={t} style={{ fontSize: "clamp(11px,1.2vw,13px)", fontWeight: 600, color: "#92400E" }}>{t}</div>
        ))}
      </div>
      ...
      <div style={{ padding: "clamp(40px,5vw,64px) clamp(20px,4vw,48px)" }}>
        ...
        <p style={{ fontSize: 14, color: "#6B7280", textAlign: "center", maxWidth: 560, margin: "0 auto 40px", lineHeight: 1.7 }}>Whether you are a business owner or an individual — we have a service that fits your stage and budget.</p>

        {/* 4-column fixed grid */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, maxWidth: 1200, margin: "0 auto" }}>
          {SERVICES.map(svc => (
            <div key={svc.id} style={{ borderRadius: 14, border: "1px solid #E5E7EB", background: "#fff", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                ...
                {svc.items.map(it => (
                  <div key={it.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid #F9FAFB" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      {it.sample && (
                        <button onClick={() => navigate(it.sample)} style={{ fontSize: 9, color: svc.btnColor, background: svc.btnBg, border: `1px solid ${svc.btnBorder}`, borderRadius: 3, padding: "1px 5px", cursor: "pointer", whiteSpace: "nowrap" }}>
                          Sample
                        </button>
                      )}
                    </div>
              ...
              <div style={{ padding: "12px 18px 18px" }}>
                <button onClick={() => navigate(svc.page)} style={{ display: "block", width: "100%", padding: 9, borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", background: svc.btnBg, color: svc.btnColor, border: `1px solid ${svc.btnBorder}` }}>
                  {svc.btn}
                </button>
              </div>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20, maxWidth: 1000, margin: "0 auto" }}>
          {[
            ...
            <div key={w.id} style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 12, padding: 24, borderTop: `3px solid ${w.color}` }}>
              ...
              <button onClick={() => navigate(w.id)} style={{ background: "transparent", border: `1px solid ${w.color}40`, color: w.color, fontSize: 12, fontWeight: 600, padding: "8px 14px", borderRadius: 6, cursor: "pointer" }}>
                Explore {w.title} →
              </button>
            </div>
          ))}
